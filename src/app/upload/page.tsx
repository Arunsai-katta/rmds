...
export default function UploadPage() {
  const [files, setFiles] = useState<{ id: number; file: File; status: "pending" | "uploading" | "ocr" | "generating" | "done" | "error" }[]>([]);
  ...
  const handleFiles = (newFiles: FileList | null) => {
    ...
    const pdfs = Array.from(newFiles)
      ...
      .map(file => ({ id: ++idCounter.current, file, status: "pending" as const }));
  ...
  const handleParseAll = async () => {
    ...
    const updateStatus = (id: number, status: typeof files[number]["status"]) =>
      setFiles(prev => prev.map(f => f.id === id ? { ...f, status } : f));

    const processEntry = async (entry: typeof files[number]): Promise<ParsedPDFResult | null> => {
      ...
      try {
        ...
        if (data.results && data.results.length > 0) {
          ...
          if (!parsedRes.referringPhysicianName || parsedRes.referringPhysicianName.trim() === "") {
            updateStatus(entry.id, "ocr");
            try {
              const pdfjsLib = await import("pdfjs-dist");
              pdfjsLib.GlobalWorkerOptions.workerSrc = '//cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';
              const arrayBuffer = await entry.file.arrayBuffer();
              const pdfDoc = await (await pdfjsLib.getDocument({ data: arrayBuffer }).promise);
              ...
              if (context) {
                ...
                const ocrRes = await fetch("/api/parse-ocr", {
                  ...
                  body: JSON.stringify({ fileName: entry.file.name, ocrText: text, pdfBase64: parsedRes.pdfBase64 })
                });
                const ocrData = await ocrRes.json();
                if (ocrData.result) parsedRes = ocrData.result;
              }
            } catch (e) {
          ...
          await fetch("/api/generate-hl7", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ parsedData: parsedRes, saveToDb: true }),
          });
    ...
    for (let i = 0; i < pending.length; i += CONCURRENCY) {
      ...
      const batchResults = await Promise.allSettled(batch.map(entry => processEntry(entry)));
            ...
            {files.map((f) => (
              <div key={f.id} className="flex items-center justify-between" style={{ padding: "8px 0", borderBottom: "1px solid var(--border-color)" }}>
                ...
                {(f.status !== "uploading" && f.status !== "ocr" && f.status !== "generating") && (
                  <button className="btn btn-danger btn-sm btn-icon" onClick={() => removeFile(f.id)}>✕</button>
                )}
              </div>
            ))}
          </div>
